...
Agent skills are instruction packages that tools like Claude Code *execute with your
credentials and filesystem*. The official `npx skills` CLI installs them well, but after
...
content (supply-chain drift), (c) a security auditor flips a skill to *warn*/*fail*, or
...
```jsonc
{
  ...
  "skills": {
    "anthropics/skills/frontend-design": {       // key = registry id, sorted
      ...
      "audits": {                                // per-partner snapshot; {} = unaudited at pin
...
- **Replacing the `npx skills` install flow.** skillwarden never installs, updates, or
  removes skills; `check` tells you *when* to go run `npx skills` commands.
